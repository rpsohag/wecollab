...
@section('content-header')
    <h1>
        Nuova Richiesta {{$tipologie_richieste[$tipologia_sel]}}
    </h1>
@stop
@section('content')
<div class="section">
	...
	{!! Form::open(['route' => 'admin.account.richieste.store', 'method' => 'post' , 'id' => "invio_richiesta"]) !!}
		<div class="box-body contenitore">
			<div class="row">
				<input type="hidden" value="{{$tipologia_sel}}" name="tipologia_sel"/>
				@if($tipologia_sel == 1)
					<div class="col-md-3">
						{!! Form::weDate('from' , 'Data Di Inizio *', $errors ,null) !!}
					</div>
					<div class="col-md-3">
						{!! Form::weDate('to' , 'Data Di Fine *', $errors ,null) !!}
					</div>
					<div class="col-md-6">
						{!! Form::weText('note' , 'Note', $errors ,null) !!}
					</div>
				@endif
				...
				@if($tipologia_sel == 2)
					...
					<div class="col-md-2">
						{!! Form::weTimeRichieste('ora_inizio' , 'Ora Di Inizio *', $errors ,null,['id'=>'ora_inizio']) !!}
					</div>
					<div class="col-md-2">
						{!! Form::weTimeRichieste('ora_fine' , 'Ora Di Fine *', $errors ,null,['id'=>'ora_fine']) !!}
					</div>
					<div class="col-md-2">
						{!! Form::weInt('num_ore' , 'Numero di Ore', $errors ,null,['readonly'=>'readonly','id'=>'num_ore' , 'step' => 1 , 'max'=> '7' , 'min' => '1']) !!}
					</div>
					<div class="col-md-4">
						{!! Form::weSelectSearch('tipo_permesso' , 'Tipo Permesso *', $errors , $tipologie_permessi,1) !!}
					</div>
					<div class="col-md-12">
						{!! Form::weText('note' , 'Note', $errors ,null) !!}
					</div>
				...
				@if($tipologia_sel == 3)
					...
					<div class="col-md-12">
						{!! Form::weText('note' , 'Note / Numero Protocollo Malattia', $errors ,null) !!}
					</div>
				@endif

				@if($tipologia_sel == 4)
					<div class="col-md-3">
						{!! Form::weSelectSearch('mese' , 'Mese *', $errors , $mesi,$mese_help) !!}
					</div>
					<div class="col-md-3">
						{!! Form::weSelectSearch('anno' , 'Anno *', $errors , $anni,date('Y')) !!}
					</div>
					<div class="col-md-3">
						<br>
		...
		<div class="box-footer">
				...
				@if($tipologia_sel == 5 || $tipologia_sel == 4)
					<button id="btn_bozza" type="button" class="btn btn-default btn-flat"><i class="fa fa-bolt"></i> Salva</button>
				@endif
		</div>
	{!! Form::close() !!}
	</div>
	...
	<div class="row">
		<div class="col-md-4 col-sm-6 col-xs-12">
			<div class="info-box">
				...
				<span id="{{$tipologia_sel == 4 ? 'tot_da_pagare' : 'tot_rimborso'}}" class="info-box-number"></span>
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCKSLtAPDDZ9CM33kvvV_oXHPNj7h5ppvU"></script>
    <script>
        ...
        $( document ).ready(function() {
			...
			$('#ora_inizio , #ora_fine').focusout(function() {
				var ora_inizio = $('#ora_inizio').val();
				var ora_fine = $('#ora_fine').val();
				var ore_diff = time_diff(ora_inizio, ora_fine);
				if(ore_diff > 7)
					ore_diff = 7;
				$("#num_ore").val(ore_diff);
			});
			...
			$("#btn_bozza").click(function(){
				var elem ="<input type='hidden' value='bozza' name='bozza'/><input type='hidden' value='{{$tipologia_sel}}' name='tipologia_sel' ";
		...
		function StartTrasferte()
		{
			$("#tot_da_pagare").text('0,00 €');
		}

		function StartKm()
		{
			$("#tot_rimborso").text('0,00 €');
		...
		function CalcoloTrasferte()
		{
			...
			$("#tot_da_pagare").text(formatter.format(rimb_spese));
		}
