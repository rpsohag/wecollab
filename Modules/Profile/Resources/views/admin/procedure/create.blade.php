@php
    $procedura = (empty($procedura)) ? '' : $procedura;
@endphp
...
@section('content-header')
    <h1>
        {{ trans('profile::procedure.title.create procedura') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.profile.procedure.index') }}">{{ trans('profile::procedure.title.procedure') }}</a></li>
        <li class="active">{{ trans('profile::procedure.title.create procedura') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.profile.procedura.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
